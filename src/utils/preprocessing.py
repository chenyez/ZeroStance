...
# Data Loading
def load_data(filename, dataset, dataset_for_test):
    ...
    if 'train' not in filename and dataset=='vast':
...
# Clean All Data
def clean_all(filename, norm_dict, dataset, dataset_for_test):
    
    concat_text = load_data(filename, dataset, dataset_for_test) # load all data as DataFrame type
    ...
    noun_phrase_datasets = ["vast", "covid19", "semeval2016", "pstance", "wtwt"]
    for i in range(len(raw_data)):   
        if dataset in noun_phrase_datasets:
    ...
    # remove empty texts
    rm_ind = [ind for ind in range(len(clean_data)) if clean_data[ind] == []]
    clean_data = [j for i, j in enumerate(clean_data) if i not in rm_ind]
    label = [j for i, j in enumerate(label) if i not in rm_ind]
    x_target = [j for i, j in enumerate(x_target) if i not in rm_ind]
